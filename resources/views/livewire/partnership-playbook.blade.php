<div class="h-full bg-[#FDFBF7] rounded-[2.5rem] shadow-[0_8px_30px_rgb(0,0,0,0.04)] border border-[#E5E0D0] p-8 flex flex-col relative overflow-hidden group text-[#2A3C2A]">
    ...
    <div class="flex justify-between items-start mb-8 relative z-10">
        <div>
            <h3 class="text-3xl font-serif font-bold tracking-tight mb-2 text-[#2A3C2A]">Daily Wisdom</h3>
            <p class="text-[#6B7C6B] text-sm font-medium tracking-wide uppercase">Curated for you</p>
        </div>
    ...
    @if($insight)
        <div class="flex-grow flex flex-col relative z-10">
            <span class="inline-block py-1.5 px-4 rounded-full text-[10px] font-bold uppercase tracking-widest mb-6 w-fit border {{ $insight->trigger_context === 'low_energy' ? 'bg-[#D86C58]/10 text-[#D86C58] border-[#D86C58]/20' : 'bg-[#4A6741]/10 text-[#4A6741] border-[#4A6741]/20' }}">
                {{ $insight->trigger_context === 'low_energy' ? 'Rest & Recovery' : 'Daily Spark' }}
            </span>
            ...
            @if($insight->type === 'audio')
                <div class="bg-white/60 backdrop-blur-md rounded-2xl p-4 flex items-center gap-4 border border-[#E5E0D0] hover:bg-white/80 transition-colors cursor-pointer group/audio shadow-sm">
                    ...
                    <div class="flex-grow">
                        <div class="text-xs font-bold text-[#2A3C2A] mb-1 uppercase tracking-wider">Play Audio</div>
                    ...
                    <div class="relative transition-all duration-500 min-h-[140px] flex flex-col justify-center">
                        @if(!$amoraResponse && !$isLoading)
                            @if($amoraMode === 'INTERPRETER')
                                <div class="space-y-3">
                                    <textarea wire:model="userComplaint" class="w-full bg-[#2C3E2C]/50 rounded-xl border-none text-sm text-[#E8E6D9] placeholder-[#5C6B5C] focus:ring-1 focus:ring-[#8F9E8F] resize-none h-16 p-3" placeholder="Analyze a situation..."></textarea>
                                    <button wire:click="askAmora" class="w-full bg-[#4A6741] hover:bg-[#5C7C53] text-white font-bold py-3 rounded-xl text-xs uppercase tracking-wider transition-all shadow-lg flex items-center justify-center gap-2">
                                        Analyze <svg class="w-3 h-3" fill="none" stroke="currentColor" viewBox="0 0 24 24"><path stroke-width="2" d="M14 5l7 7m0 0l-7 7m7-7H3"></path></svg>
                                    </button>
                                </div>
                        ...
                        @elseif($isLoading)
                            <div class="flex flex-col items-center justify-center space-y-3">
                                ...
                                <p class="text-[10px] font-bold text-[#8F9E8F] uppercase tracking-widest animate-pulse">Consulting...</p>
                            </div>
                            ...
                            @if($amoraMode === 'INTERPRETER')
                                <div class="animate-fadeIn">
                                    <div class="flex items-center gap-2 mb-3">
                                         ...
                                         <span class="text-[10px] font-bold uppercase tracking-wider text-[#8F9E8F]">Risk: {{ $amoraResponse['risk_level'] ?? 'Low' }}</span>
                                    </div>
                            ...
                            @elseif($amoraMode === 'SPARK')
                                <div class="text-center animate-fadeIn">
                                    ...
                                    <button wire:click="askAmora" class="text-[10px] font-bold text-[#5C6B5C] hover:text-[#E8E6D9] uppercase tracking-widest flex items-center justify-center gap-1 mx-auto">
                                        <svg class="w-3 h-3" fill="none" stroke="currentColor" viewBox="0 0 24 24"><path stroke-width="2" d="M4 4v5h.582m15.356 2A8.001 8.001 0 004.582 9m0 0H9m11 11v-5h-.581m0 0a8.003 8.003 0 01-15.357-2m15.357 2H15"></path></svg> New Spark
                                    </button>
                                </div>
                            @endif
                        @endif
